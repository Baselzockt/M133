...
function loadAllJobs() {
    //load all jobs
    $.getJSON("//sandbox.gibm.ch/berufe.php").done(data => {
...
function getClasses(jobId) {
    $.getJSON("//sandbox.gibm.ch/klassen.php?beruf_id=" + jobId,
        data => {
            if (data.length > 0) {
...
function getTimeTable(classId) {
    ...
    $.getJSON("//sandbox.gibm.ch/tafel.php?klasse_id=" + classId + "&woche=" + date.getWeek() + "-" + date.getFullYear(),
        function(data) {
